...
{
	if (object->IsActive()) {
		ApiEvents::GetProcessResultHelper(object, EventType::ObjectCreated, "ObjectCreated");
	} else if (!object->IsActive() && !object->GetExtension("ConfigObjectDeleted").IsEmpty()) {
		ApiEvents::GetProcessResultHelper(object, EventType::ObjectDeleted, "ObjectDeleted");
	}
}

void ApiEvents::OnVersionChangedHandler(const ConfigObject::Ptr& object, const Value&)
{
	ApiEvents::GetProcessResultHelper(object, EventType::ObjectModified, "ObjectModified");
}

void ApiEvents::GetProcessResultHelper(const ConfigObject::Ptr &object, const EventType &eventType, const String& eventQueue) {
